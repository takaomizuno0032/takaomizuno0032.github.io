...
To solve this issue, we developed a platform for storing and updating LiDAR sensor data on a distributed ledger in real-time as part of a commissioned research project by the National Institute of Information and Communications Technology (NICT). The project has received the highest evaluation ranking from NICT in 2023.
...
Additionally, to further improve data reliability, we developed a system in which nodes autonomously proliferate to run the distributed ledger at each intersection.
...
Lab: The University of Queensland, UQ Cyber

Mentor: Prof. Ryan Ko

With the recent rise of cryptocurrency, the damage caused by ransomware has been increasing. Ransomware is a type of malware that infects computers, encrypts data, and demands a decryption key. Victims are required to make payments to obtain the decryption key, and due to its high level of anonymity, Bitcoin is often used for these payments.
Existing research has focused on identifying transactions used for ransomware payments by analyzing Bitcoin transactions. However, due to the high level of anonymity in Bitcoin transactions, it is challenging to identify them.
In this research, we proposed a method that leverages large language models to aggregate external information and de-anonymize Bitcoin transactions.

The research results were presented at an internal research symposium and nominated for the Best Thesis Award.
